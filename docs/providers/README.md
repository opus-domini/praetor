...
Praetor uses a single canonical provider abstraction: `internal/agent.Agent`.
...
| OpenRouter | REST | `https://openrouter.ai/api/v1` |
| Ollama | REST | `http://127.0.0.1:11434` |

## Adding a new provider

1. Implement a new `internal/agent.Agent` adapter (CLI or REST).
2. Register it in `internal/agent/runtime.NewDefaultRegistry(...)`.
