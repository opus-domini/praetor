...
func (r *TransitionRecorder) WriteMetric(entry domain.CostEntry) error {
...
func (r *TransitionRecorder) CompleteTask(state *domain.State, index int, signature, runID, message string) error {
	task := &state.Tasks[index]
	if err := domain.Transition(task.Status, domain.TaskDone); err != nil {
		return err
	}
...
func (p *IsolationPolicy) RollbackTask(ctx context.Context, runID string, render domain.RenderSink) {
