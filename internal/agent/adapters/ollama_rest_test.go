...
func TestOllamaRESTExecute(t *testing.T) {
	...
	provider := NewOllamaREST(server.URL, "llama3.1", server.Client())
	resp, err := provider.Execute(context.Background(), agent.ExecuteRequest{Prompt: "hi"})
	if err != nil {
...
func TestOllamaRESTPlanExtractsManifest(t *testing.T) {
	...
	provider := NewOllamaREST(server.URL, "llama3.1", server.Client())
	resp, err := provider.Plan(context.Background(), agent.PlanRequest{Objective: "build"})
	if err != nil {
		t.Fatalf("plan: %v", err)
	}
